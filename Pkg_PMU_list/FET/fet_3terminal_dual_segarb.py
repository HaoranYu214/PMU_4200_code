...
from debug.waveform_preview import (
...
# standalone timestamped filename behavior.
OUTPUT_TAG = None
...
MAX_SEGMENTS_PER_SEQUENCE = 1048
...
def main():
    ...
    with PMUSession(INST, channels=(GATE_CH, DRAIN_CH)) as session:
        ...
        try:
            ...
            data = add_derived_fet_columns(pd.concat([plan, measured], axis=1))
            if OUTPUT_TAG:
                path = SAVE_DIR / f"FeFET_dual_{OUTPUT_TAG}.xlsx"
            else:
                stamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
                path = SAVE_DIR / f"FeFET_dual_program_read_segarb_{stamp}.xlsx"
            save_fet_workbook(
                path,
                data,
                {
