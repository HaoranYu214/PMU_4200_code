...
class FeCapEntryWiringTests(unittest.TestCase):
    ...
    def test_sweep_exposes_and_propagates_common_options(self):
        path = FE_CAP_ROOT / "PV2_PUND_tri_sweep.py"
        source = path.read_text(encoding="utf-8")
        tree = ast.parse(source, filename=str(path))
        options = segarb_options_literal(tree)
        self.assertTrue(REQUIRED_OPTIONS.issubset(options))
        self.assertIn("module.SEGARB_OPTIONS.clear()", source)
        self.assertIn("module.SEGARB_OPTIONS.update(SEGARB_OPTIONS)", source)
