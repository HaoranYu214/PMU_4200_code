...
class WaveformOwnershipTests(unittest.TestCase):
    def test_shared_pmu_module_contains_no_experiment_protocols(self):
        functions = top_level_functions("src/pmu/pmu_tests.py")
        hidden_protocols = {
    ...
    def test_fet_entries_own_their_pulse_protocols(self):
        self.assertIn(
            ...
            top_level_functions("Pkg_PMU_list/FET/PlsTrn.py"),
        )
        self.assertIn(
            "run_dual_channel_sweep_train",
            top_level_functions("Pkg_PMU_list/FET/C1swp_C2Trn.py"),
        )

    def test_segment_arb_waveforms_are_owned_by_experiment_entries(self):
        expected_builders = {
            "Pkg_PMU_list/Fe_cap/PV2.py": {"make_pv2_seq_configs"},
            "Pkg_PMU_list/Fe_cap/PUND_test.py": {"make_pund_seq_configs"},
            "Pkg_PMU_list/Fe_cap/endurance.py": {
                "make_cycle_seq_configs",
                "make_pv2_seq_configs",
                "make_pund_seq_configs",
            },
            "Pkg_PMU_list/Programed_measuremnts/NLS_1C_switch.py": {
