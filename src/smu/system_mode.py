...
def run_linear_voltage_sweep(
    ...
    *,
    ...
    try:
        ...
        resolved_timeout_s = resolve_smu_timeout_s(
            timeout_s,
            point_count=linear_sweep_point_count(start, stop, step),
            hold_time=hold_time,
            sweep_delay=sweep_delay,
        ...
        execute_and_wait(query, execution_mode=1, timeout_s=resolved_timeout_s)
        restore_rpms_to_pulse(query, rpm_targets)
        return list(return_variables)
    except BaseException:
        shutdown_system_mode(query, available_channels)
        # Do not move an RPM relay after an execution/setup failure: output
        # state may be uncertain even after the best-effort abort. Leaving the
        # RPM on the SMU path is safer than switching an energized source.
        raise
...
def run_list_voltage_sweep(
    ...
    *,
    ...
    try:
        ...
        shutdown_system_mode(query, available_channels)
        # Do not move an RPM relay after an execution/setup failure: output
        # state may be uncertain even after the best-effort abort.
        raise
